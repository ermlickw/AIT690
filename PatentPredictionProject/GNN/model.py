...
class graph2graph(object):
    def __init__(self, sess,Ds,Ds_label, Dr,De_o,checkpoint_dir,epoch,type_):
        self.sess = sess
        self.Ds = Ds
    ...
    def test(self, args):
        ...
        O_target=label
        Ra=Ra_data
        node,label=self.sess.run([self.O_p,self.O_target],feed_dict={self.O:O,self.O_target:O_target,self.Ra:Ra,self.Rr:Rr_data,self.Rs:Rs_data,self.index:0});
        print(" test acc: "+str(acc(node[:,train_len:],label[:,train_len:])));
